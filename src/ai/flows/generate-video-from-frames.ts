...
/**
 * @fileOverview Generates a video by animating a still avatar image based on a first and last frame.
 *
 * - generateVideoFromFrames - A function that handles the video generation process.
 * - GenerateVideoFromFramesInput - The input type for the function.
 * - GenerateVideoFromFramesOutput - The return type for the function.
 */
...
const GenerateVideoFromFramesInputSchema = z.object({
  ...
  firstFrameDataUri: z
    .string()
    .describe(
      "The starting frame of the motion, as a data URI."
    ),
   lastFrameDataUri: z
    .string()
    .describe(
...
const GenerateVideoFromFramesOutputSchema = z.object({
  generatedVideoDataUri: z
    .string()
    .describe('The generated video of the avatar animated, as a data URI.'),
});
export type GenerateVideoFromFramesOutput = z.infer<typeof GenerateVideoFromFramesOutputSchema>;

export async function generateVideoFromFrames(input: GenerateVideoFromFramesInput): Promise<GenerateVideoFromFramesOutput> {
  return generateVideoFromFramesFlow(input);
}

const generateVideoFromFramesFlow = ai.defineFlow(
  {
    name: 'generateVideoFromFramesFlow',
